...
func main() {
	...
	// 启动加载配置和监听
	nacosManager.Start("orderhttp.env")
	// 加载环境变量
	err := godotenv.Load("./orderhttp.env")
	if err != nil {
		fmt.Println("Error loading .env file", err)
	}

	// 初始化Hertz服务器并设置端口为8000
	h := server.Default(server.WithHostPorts("0.0.0.0:8003"))
	...
	//strport := os.Getenv("ORDER_HTTP_PORT")
	...
	url := swagger.URL("http://localhost:8003/swagger/index.html")
