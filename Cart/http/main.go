...
func main() {
	...
	// 启动加载配置和监听
	nacosManager.Start("carthttp.env")
	// 加载环境变量
	err := godotenv.Load("./carthttp.env")
	if err != nil {
		fmt.Println("Error loading .env file", err)
	}

	// 初始化Hertz服务器并设置端口为8000
	h := server.Default(server.WithHostPorts("0.0.0.0:8002"))
	...
	//strport := os.Getenv("CART_HTTP_PROT")
	...
	apiPaths := []string{
		"POST:/cart/AddCart",
		"GET:/cart/GetCart",
		"PUT:/cart/emptyCart/:userId",
	}
	registerFlowRules(apiPaths, 10, 1000)

	url := swagger.URL("http://localhost:8002/swagger/index.html")
